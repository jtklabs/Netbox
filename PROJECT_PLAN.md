...
Cross-stream: the SNMPv3 inventory scanner (`scripts/snmp-inventory/`, separate work stream) pushes readings to `POST /api/plugins/refresh/device-software/report/` — single object or list, device by id or name, version **raw and unnormalised**, empty meaning "not collected" rather than a placeholder. Its `DeviceFacts.software_version` / `VendorProfile.platform` line up with that shape.
