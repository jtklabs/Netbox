...
- **Images are linked, not proxied.** Images run 300MB–1GB and the bytes live on an internal HTTP file server; we store filename, size, checksum(+type) and a URL, with an optional FileField for the cases where NetBox should hold them. A plain `http://` link from the https UI is fine — it is a top-level navigation, not blocked mixed content — provided nothing ever *fetches* it from JavaScript. (For reference: prod's default storage is S3 with django-storages' `querystring_auth` default, so a FileField URL there is already a presigned direct-to-S3 link; the web tier only ever proxies the *upload*, which is the expensive direction.)
