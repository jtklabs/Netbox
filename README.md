...
- `scripts/prepare-docker-host.sh` — turn a fresh Ubuntu 24 / RHEL 9 box into a Docker host for these stacks: engine + Compose v2, **all Docker networking pinned to CGNAT** (100.64.0.0/10 — the 172.17/12 defaults collide with our networks), optional deploy-key setup for unattended collector pushes
